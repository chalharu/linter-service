	...
	try {
		...
		assert.equal(
			...
			[
				...
				"| --- | --- | --- |",
				"| `actionlint` | ✅ Pass | No issues were reported for the selected GitHub Actions workflow target(s). |",
				"| `rustfmt` | ❌ Issues | Rust formatting issues were detected. |",
				"",
...
test("falls back to decrypt failure messaging when detailed summaries are unavailable", () => {
	...
	try {
		...
		assert.match(
			commentBody,
			/\| `cargo-deny` \| ❌ Missing summary \| The encrypted `cargo-deny` summary could not be decrypted\. See the workflow logs\. \|/,
		);
