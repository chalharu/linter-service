...
| `textlint` | `*.md`, `*.markdown`, `*.txt` | repo root の static JSON `.textlintrc` | default では無効です。`.github/linter-service.json` で `linters.textlint.disabled: false` と exact version 付き `preset_package` を指定した時だけ動作します。shared workflow は repo code を読まないため、YAML / JS / comment 付き config は未対応です。 |
...
- `textlint` は default で無効です。使う場合は `disabled: false` を明示し、
  `preset_package` に exact version 付き npm package spec を指定してください。
